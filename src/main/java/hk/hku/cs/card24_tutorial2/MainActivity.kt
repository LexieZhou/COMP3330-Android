package hk.hku.cs.card24_tutorial2
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        checkInput!!.setOnClickListener {
            ...
            if (total != 4) {
                Toast.makeText(
                    this@MainActivity, "Please select all cards",
                    Toast.LENGTH_SHORT
                ).show()
            }
            else {
    ...
    private fun initCardImage() { for (i in 0..3) {
        val resID: Int = resources.getIdentifier(
            "back_0",
            "drawable",
            "hk.hku.cs.card24_tutorial2")
    ...
    private fun pickCard() {
        ...
        for (i in 0 until 4) {
            ...
            var randomNumber = Random.nextInt(1, 14)
            while (randomNumber in generatedNumbers) {
                randomNumber = Random.nextInt(1, 14)
            }
            generatedNumbers.add(randomNumber)
            data[i] = randomNumber
            // assign card
            var suit = Random.nextInt(0, 3)
...
//        data[3] = 10
    ...
    private fun setClear() {
        ...
        for (i in 0..3) {
            imageCount[i] = 0
            resID = resources.getIdentifier("card" + card[i], "drawable", "hk.hku.cs.card24_tutorial2")
            cards[i].setImageResource(resID)
